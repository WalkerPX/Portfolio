...
const Penguin = ({ isMuted, isDark, onHover }: PenguinProps) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [hovered, setHovered] = useState(false);
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const [bobY, setBobY] = useState(0);

  // ✅ SAFE animation loop (no freezing)
  useEffect(() => {
    let t = 0;
    let frameId: number;
    ...
    >
        ...
        NOTE_POSITIONS.map((n, i) => (
          <div
            ...
            style={{
              left: `calc(50% + ${n.x}px)`,
              bottom: "90px",
              animation: `floatNote ${n.dur} ${n.delay} infinite ease-in-out`,
              opacity: 0,
            }}
          >
            ♪
      ...
      <div className="relative flex flex-col items-center">
        <div
          style={{
            animation: isPlaying
              ? "penguinSway 1.2s ease-in-out infinite alternate"
              : "none",
